...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    private ArrayList<Integer> mVisibleEventIndices = new ArrayList<Integer>();
    ...
    */
    ...
    private static int STAGGERED_EVENT_SHADOW_WIDTH = 4;
    ...
    private static int mStaggeredEventShadowColor;
    ...
    private boolean mDrawStaggered = false;
    ...
    public DayView(Context context, CalendarController controller,
        ...
        if (mScale == 0) {
            ...
            if (mScale != 1) {
                ...
                STAGGERED_EVENT_SHADOW_WIDTH *= mScale;
    ...
    private void init(Context context) {
        ...
        mStaggeredEventShadowColor = DynamicThemeKt.getColor(mContext, "staggered_event_shadow_color");
    ...
    public void handleOnResume() {
        ...
        for (int h = 0; h < 24; h++) {
            if (mIs24HourFormat) {
                mHourStrs[h] = String.format("%02d", h);
            } else {
                int hour12 = h % 12;
                if (hour12 == 0) hour12 = 12;
                String numStr = String.format("%02d", hour12);
                if (h == 5) {
                    mHourStrs[h] = numStr + "\n上午";
                } else if (h == 12) {
                    mHourStrs[h] = numStr + "\n下午";
    ...
    private void remeasure(int width, int height) {
        ...
        // ==================== 修改核心起点 ====================
        // 强制设置全天/天气区域的最小高度（28dp），防止没有全天日程时高度变为 0
        int minWeatherHeight = (int) (38 * mScale);
        if (allDayHeight < minWeatherHeight) {
            allDayHeight = minWeatherHeight;
        }

        // 根据最终的 allDayHeight 统一计算主网格起点坐标
        mFirstCell = DAY_HEADER_HEIGHT + allDayHeight + ALLDAY_TOP_MARGIN;
        // ==================== 修改核心终点 ====================
        ...
        SharedPreferences prefs = GeneralPreferences.Companion.getSharedPreferences(mContext);
        mDrawStaggered = prefs.getBoolean(GeneralPreferences.KEY_STAGGERED_DISPLAY, false);
        Event.computePositions(mEvents, minimumDurationMillis, mDrawStaggered);
...
//        mContext.startProgressSpinner();
    ...
    private void drawHours(Rect r, Canvas canvas, Paint p) {
        ...
        int mainTextColor = isDarkMode ? 0xFFEEEEEE : 0xFF333333; // 数字颜色
        int subTextColor  = isDarkMode ? 0xFF888888 : 0xFF777777; // 上午/下午颜色

        // ==================== [核心逻辑 1] 动态计算“上午”与“下午”当前应挂载的目标时刻 ====================
        int amTargetHour = -1;
        int pmTargetHour = -1;

        // 寻找 AM (1~11点) 中第一个未滑出顶部的时刻（初始为 1 点，滑出后自动后移）
        for (int h = 1; h <= 11; h += hourStep) {
            int hourY = h * totCellHeight + mHoursTextHeight / 2 - HOUR_GAP;
            if (hourY >= mViewStartY) {
                amTargetHour = h;
                break;
            }
        }

        // 寻找 PM (12~23点) 中第一个未滑出顶部的时刻（初始为 12 点，滑出后自动后移）
        for (int h = 12; h <= 23; h += hourStep) {
            int hourY = h * totCellHeight + mHoursTextHeight / 2 - HOUR_GAP;
            if (hourY >= mViewStartY) {
                pmTargetHour = h;
                break;
        ...
        for (; i < 24; i += hourStep) {
            ...
            // 提取纯数字部分（如“01”、“12”）
            String numberStr = time.contains("\n") ? time.split("\n")[0] : time;

            // ==================== [核心逻辑 2] 判断当前时刻是否需要附加“上午”/“下午” ====================
            String subText = null;
            if (i == amTargetHour) {
                subText = "上午";
            } else if (i == pmTargetHour) {
                subText = "下午";
            }
            // ==============================================================================================

            if (subText != null) {
                // 绘制数字（位置稍微上移 2px，给下面的文字留空间）
                p.setColor(mainTextColor);
                canvas.drawText(numberStr, centerX, y - 2, p);

                // 绘制动态出现的“上午” / “下午”
                float originalSize = p.getTextSize();
                p.setTextSize(originalSize * 0.6f);
                p.setColor(subTextColor);
                canvas.drawText(subText, centerX, y + p.getTextSize() + 2, p);
                ...
                // 普通时刻只画数字，垂直居中
                p.setColor(mainTextColor);
                canvas.drawText(numberStr, centerX, y, p);
            }
    ...
    private void setupHourTextPaint(Paint p) {
        p.setColor(0xFF333333);  // 索尼原版数字色
        p.setTextSize(HOURS_TEXT_SIZE);
        p.setTypeface(Typeface.DEFAULT);
        p.setTextAlign(Paint.Align.CENTER);  // 居中对齐
        p.setAntiAlias(true);
    }
    ...
    private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas, Paint p) {
        ...
        // 判断周末
        int currentJulianDay = mTodayJulianDay - mFirstJulianDay;
        int dayOfWeek = (currentJulianDay + 3) % 7;
        boolean isWeekend = (dayOfWeek == 5 || dayOfWeek == 6)
                || dayStr.contains("六") || dayStr.contains("日")
                || dayStr.toLowerCase().contains("sat") || dayStr.toLowerCase().contains("sun");

        // 索尼标准配色
        final int COLOR_SONY_RED = 0xFFD32F2F;  // 今日/周末红
        final int COLOR_DARK_TEXT = 0xFF333333; // 工作日主文字
        final int COLOR_GRAY_TEXT = 0xFF777777; // 农历灰

        if (mNumDays > 1) {
            float leftPos = computeDayLeftPosition(day);
            float rightPos = computeDayLeftPosition(day + 1);
            float centerX = (leftPos + rightPos) / 2f;

            p.setTextAlign(Paint.Align.CENTER);

            // ----------------- 精准三行纵向坐标排版 -----------------
            float yWeek = 18 * mScale;  // 第一行：周几
            float yDate = 38 * mScale;  // 第二行：日期数字
            float yLunar = 56 * mScale; // 第三行：农历

            // 1. 绘制 "周几"
            p.setTextSize(DAY_HEADER_FONT_SIZE);
            p.setTypeface(Typeface.DEFAULT);
            p.setColor((todayIndex == day || isWeekend) ? COLOR_SONY_RED : COLOR_DARK_TEXT);
            canvas.drawText(dayStr, centerX, yWeek, p);
            ...
            if (todayIndex == day) {
                ...
                circlePaint.setColor(COLOR_SONY_RED);

                float circleY = yDate - (DATE_HEADER_FONT_SIZE * 0.32f);
                float radius = DATE_HEADER_FONT_SIZE * 0.70f;
                canvas.drawCircle(centerX, circleY, radius, circlePaint);

                p.setColor(0xFFFFFFFF);

            } else if (isWeekend) {
                p.setColor(COLOR_SONY_RED);
            } else {
                p.setColor(COLOR_DARK_TEXT);
            }
            canvas.drawText(dateNumStr, centerX, yDate, p);
            ...
            // 调用新版 LunarUtils 接口获取农历短文本 (初一时显示月份，其余显示农历日)
            String lunarStr = LunarUtils.get(mContext, year, month, dateNum, LunarUtils.FORMAT_LUNAR_SHORT, false, null);

            if (!TextUtils.isEmpty(lunarStr)) {
                p.setTextSize(DAY_HEADER_FONT_SIZE - 2);
                p.setTypeface(Typeface.DEFAULT);
                p.setColor((todayIndex == day || isWeekend) ? COLOR_SONY_RED : COLOR_GRAY_TEXT);
                canvas.drawText(lunarStr, centerX, yLunar, p);
            }
        }
        // ----------------- 表头底部分割线 -----------------
        Paint linePaint = new Paint();
        linePaint.setColor(mCalendarGridLineInnerVerticalColor != 0 ? mCalendarGridLineInnerVerticalColor : 0x33FFFFFF); // 使用网格同款线色
        linePaint.setStrokeWidth(1);
        float lineY = DAY_HEADER_HEIGHT - 1;
        canvas.drawLine(0, lineY, mViewWidth, lineY, linePaint);

        // 在 drawDayHeader 方法底部：
        boolean isDarkModeHeader = (getContext().getResources().getConfiguration().uiMode
                & android.content.res.Configuration.UI_MODE_NIGHT_MASK)
                == android.content.res.Configuration.UI_MODE_NIGHT_YES;

        // 顶部阴影：高度调缩至 2.5dp，透明度大幅降低（深色 0x1F ~12%，浅色 0x10 ~6%）
        int topShadowAlpha = isDarkModeHeader ? 0x1F : 0x10;
        drawDropShadow(canvas, 0, 2.5f * mScale, topShadowAlpha);
    }
    ...
    private boolean getEventTextRect(Rect r, Event event) {
        r.top = (int) event.textTop + EVENT_RECT_TOP_MARGIN;
        ...
        // Add margin if there is enough space
    ...
    private boolean getAllDayEventTextRect(Rect r, Event event) {
    ...
    private void drawAllDayEvents(int firstDay, int numDays, Canvas canvas, Paint p) {
        ...
        // ==================== [索尼风格] 顶部 24h 面板与 Weather 栏绘制 ====================
        // 锁定天气栏的单行显示高度（28dp），防止多日程拉高 mAlldayHeight 时图标垂直偏位
        final float weatherBarHeight = 38 * mScale;
        final float weatherCenterY = startY + (weatherBarHeight / 2f);

        // 判断是否为深色模式/暗色背景（也可根据 mBgColor 或系统主题判断）
        boolean isDarkMode = (getContext().getResources().getConfiguration().uiMode
                & android.content.res.Configuration.UI_MODE_NIGHT_MASK)
                == android.content.res.Configuration.UI_MODE_NIGHT_YES;
        ...
        bgPaint.setColor(isDarkMode ? 0xFF1E1E1E : 0xFFF5F5F5); // 右侧天气背景
        canvas.drawRect(0, startY, mViewWidth, startY + weatherBarHeight, bgPaint);

        Paint leftBgPaint = new Paint();
        leftBgPaint.setStyle(Style.FILL);
        leftBgPaint.setColor(isDarkMode ? 0xFF2A2A2A : 0xFFEAE8E7); // 左侧 24h 块背景
        canvas.drawRect(0, startY, mHoursWidth, startY + weatherBarHeight, leftBgPaint);

        // 2. 绘制左侧 24h 标识 (带 drawable 缺失时的兜底文字)
        Drawable icon24h = null;
        try {
            icon24h = getContext().getDrawable(R.drawable.ic_all_day);
        } catch (Exception ignored) {}

        if (icon24h != null) {
            int iconSize = (int) (18 * mScale);
            int left = (mHoursWidth - iconSize) / 2;
            int top = (int) (weatherCenterY - (iconSize / 2f));
            icon24h.setBounds(left, top, left + iconSize, top + iconSize);
            if (isDarkMode) icon24h.setTint(0xFFDDDDDD);
            icon24h.draw(canvas);
        } else {
            // 【兜底绘制】：若无图片资源，直接用 Bold 文字绘制 "24h"
            Paint text24hPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            text24hPaint.setColor(isDarkMode ? 0xFFCCCCCC : 0xFF555555);
            text24hPaint.setTextSize(12 * mScale);
            text24hPaint.setTypeface(Typeface.DEFAULT_BOLD);
            text24hPaint.setTextAlign(Paint.Align.CENTER);
            canvas.drawText("24h", mHoursWidth / 2f, weatherCenterY + (text24hPaint.getTextSize() / 3f), text24hPaint);
        }

        // 3. 绘制右侧 Weather 栏 (带 drawable 缺失时的兜底小太阳)
        String weatherText = "未设定位置";
        Paint weatherPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        weatherPaint.setColor(isDarkMode ? 0xFFBBBBBB : 0xFF666666); // 高对比度灰白
        weatherPaint.setTextSize(12 * mScale);
        weatherPaint.setTextAlign(Paint.Align.RIGHT);

        float weatherRightX = mViewWidth - (14 * mScale);
        float textBaselineY = weatherCenterY + (weatherPaint.getTextSize() / 3f) - 1;

        // 绘制文字
        canvas.drawText(weatherText, weatherRightX, textBaselineY, weatherPaint);

        // 绘制天气图标
        Drawable weatherIcon = null;
        try {
            weatherIcon = getContext().getDrawable(R.drawable.no_permission_weather);
        } catch (Exception ignored) {}

        if (weatherIcon != null) {
            int textWidth = (int) weatherPaint.measureText(weatherText);
            int iconSize = (int) (16 * mScale);
            int spacing = (int) (6 * mScale);

            int iconRight = (int) weatherRightX - textWidth - spacing;
            int iconLeft = iconRight - iconSize;
            int iconTop = (int) (weatherCenterY - (iconSize / 2f));

            weatherIcon.setBounds(iconLeft, iconTop, iconRight, iconTop + iconSize);
            if (isDarkMode) weatherIcon.setTint(0xFFFFA726); // 橙色/浅色着色
            weatherIcon.draw(canvas);
        } else {
            // 【兜底绘制】：若无天气图片，在文字左侧画一个橙色太阳圆点
            Paint sunPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
            sunPaint.setColor(0xFFFFA726);
            float textWidth = weatherPaint.measureText(weatherText);
            float circleX = weatherRightX - textWidth - (12 * mScale);
            canvas.drawCircle(circleX, weatherCenterY, 4 * mScale, sunPaint);
        }

        // 4. 天气栏下方的底部分割线
        Paint weatherDivider = new Paint();
        weatherDivider.setColor(mCalendarGridLineInnerVerticalColor != 0 ? mCalendarGridLineInnerVerticalColor : 0x33FFFFFF);
        weatherDivider.setStrokeWidth(1);
        float lineY = startY + weatherBarHeight - 1;
        canvas.drawLine(0, lineY, mViewWidth, lineY, weatherDivider);
        // ====================================================================================
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            drawEventRect(event, canvas, p, (int) event.left, (int) event.top, (int) event.right, (int) event.bottom);
            Rect r = mRect;
            if (getAllDayEventTextRect(r, event)) {
        ...
        // 在 drawAllDayEvents 方法底部：
        boolean isDarkModeBar = (getContext().getResources().getConfiguration().uiMode
                & android.content.res.Configuration.UI_MODE_NIGHT_MASK)
                == android.content.res.Configuration.UI_MODE_NIGHT_YES;

        float headerBottom = DAY_HEADER_HEIGHT;
        float weatherHeight = 38 * mScale;

        // 表头 -> 天气栏（中等过渡：深色 0x33，浅色 0x1A）
        int midShadowAlpha = isDarkModeBar ? 0x33 : 0x1A;
        drawDropShadow(canvas, headerBottom, 3.5f * mScale, midShadowAlpha);

        // 天气栏 -> 日历网格（立体感稍强：深色 0x55，浅色 0x22）
        int bottomShadowAlpha = isDarkModeBar ? 0x55 : 0x22;
        drawDropShadow(canvas, headerBottom + weatherHeight, 5 * mScale, bottomShadowAlpha);
    }
    ...
    private void drawEvents(int date, int dayIndex, int top, Canvas canvas, Paint p) {
        ...
        int right = left + cellWidth - 1; // We need to subtract 1 because 1 is added to left above
        ...
        // Pre-compute rects and filter for the day
        final ArrayList<Integer> visibleEventIndices = mVisibleEventIndices;
        visibleEventIndices.clear();
        for (int i = 0; i < numEvents; i++) {
            Event event = events.get(i);
            ...
            visibleEventIndices.add(i);
        }

        if (mDrawStaggered) {
            // Sort by column
            // This makes sure that staggered events are drawn in the right order
            visibleEventIndices.sort(Comparator.comparingInt((Integer i) -> events.get(i).getColumn()));
        }

        for (int k = 0; k < visibleEventIndices.size(); k++) {
            int i = visibleEventIndices.get(k);
            Event event = events.get(i);
            ...
            drawEventRect(event, canvas, p, left, mViewStartY, right, viewEndY);

            Rect r = mRect;
            if (!getEventTextRect(r, event)) {
                continue;
            }
...
//            }
    ...
    private void drawEventRect(Event event, Canvas canvas, Paint p, int visibleLeft, int visibleTop,
        ...
        if (!event.drawAsAllday() && event.isDrawStaggered() && event.getColumn() > 0) {
            canvas.save();
...
//        }
    ...
    private void findSelectedEvent(int x, int y) {
        ...
        if (mDrawStaggered) {
            ...
            // This makes sure that staggered events are checked in the right order when tapped
            mSelectedEvents.sort(Comparator.comparingInt(event -> -event.getColumn()));
        }
    ...
    private float distanceInfluenceForSnapDuration(float f) {
        ...
        return (float) Math.sin(f);
             ...
             * 绘制向下投影阴影
             * @param canvas 画布
             * @param topY 阴影起始 Y 坐标
             * @param shadowHeight 阴影高度
             * @param alphaHex 阴影起始透明度 (十六进制，如 0x1A, 0x33, 0x66)
             */
            private void drawDropShadow(Canvas canvas, float topY, float shadowHeight, int alphaHex) {
                int startColor = (alphaHex << 24) | 0x000000; // 动态设置 ARGB 透明度
                ...
                canvas.drawRect(0, topY, getWidth(), topY + shadowHeight, shadowPaint);
            }
}
