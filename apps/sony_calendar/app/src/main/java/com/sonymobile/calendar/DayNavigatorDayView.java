...
public class DayNavigatorDayView extends WeekNavigatorDayView {
    ...
    public DayNavigatorDayView(Context context, boolean z, ViewIndex viewIndex, FocusedViewNavigator focusedViewNavigator) {
    ...
    protected void drawDayNumber(Canvas canvas, int i, int i2) {
        ...
        if (Utils.isDateToday(this.day) && !this.isSelected) {
            Time time = new SafeTime(this.day.timezone);
            time.setToNow();
            i2 = UiUtils.getDateBoxForegroundColor(getContext(), this.day, (time.year == this.day.year && time.month == this.day.month) ? false : true, this.isSelected, Utils.isDateToday(this.day));
            i = i2;
        }
